 ...
 * representing the CSS structure by `Ekit_Scssc`. The CSS tree is fed into a
 ...
 *
 * The `parse` function of `Ekit_Scssc` is the entry point.
 *
 * In summary:
 *
 * The `Ekit_Scssc` class creates an instance of the parser, feeds it SCSS code,
...
class Ekit_Scssc {
	...
	protected function lib_rgba($args) {
		...
		list($r,$g,$b, $a) = $args;
		return array("color", isset($r[1]), isset($g[1]), isset($b[1]), isset($a[1]));
	}
...
class scss_parser {
	 ...
	 *
	 * Consider the function Ekit_Scssc::keyword(). (All parse functions are
	 * structured the same.)
	 *
	 ...
	 *
	 * All of these parse functions are powered by Ekit_Scssc::match(), which behaves
	 ...
	 * avoid this, Ekit_Scssc::seek() is used to remember and set buffer positions.
	 *
...
class scss_server {
	 ...
	 * @param \Ekit_Scssc|null $scss     SCSS compiler instance
	 */
